...
def generateFont(
	...
	**kw,
	...
	for ligature in ligatures:
		...
		createGlyph(fontList, -1, name, image, glyphclass="baseligature", **kw)
		for font in fontList:
			if font is None:
				continue
			lig = font[name]
	...
	for ligature in continuous_ligatures:
		...
		for head in ligature["heads"]:
			...
			createGlyph(fontList, -1, name_, image)
			for font in fontList:
				if font is None:
					continue
...
generateExamples(characters, ligatures, continuous_ligatures, charactersByCodepoint)
